$(function(){
    ...
    $(window).scroll(function(){
        ...
        if(moveTrue<1){
            $($scrollData).css({top:moveTrue * -200});
        }

    //img-scrollable 스크롤
        let $currentSct = $(this).scrollTop(); //스크롤양을 변수에 담기
        let $offset = 300;
        let $imgScrollable = $('.img-scrollable');
        let $imageExecuted = false

        let $imgarea = $imgScrollable.offset().top - $offset;

        if(!$imageExecuted){
            if($currentSct > $imgarea){
                $imgScrollable.addClass('reduce');
                $imageExecuted = true;
            }
        }
    });
